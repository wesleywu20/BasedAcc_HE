...
def select_numbers(notSafe = False):
    # The range is only for the following settings
    # t, n, logq, T = 64, 64, 32, 16
    # q, psi, psiv, w, wv = 4294966657, 766107853, 3158179387, 4208251219, 1307994966
    if notSafe:
        print("WARNING: Not checking range. This may result in errors.")
        
    while True:
        try:
            n1 = int(input("Enter First Number (range: +-2,147,483,647): "))
            if -1e9 <= n1 <= 1e9 or notSafe:
                break
            else:
                print("Number out of range. Please enter a number between -1e9 and 1e9.")
        except ValueError:
            print("Invalid input. Please enter an integer.")

    while True:
        try:
            n2 = int(input("Enter Second Number (range: +-2,147,483,647): "))
            if -1e9 <= n2 <= 1e9 or notSafe:
                break
            else:
                print("Number out of range. Please enter a number between -1e9 and 1e9.")
        except ValueError:
...
def encrypt(Evaluator, n1, n2):
    print("Encrypting Numbers ", n1, n2)
    ...
    # output ciphertexts and relin keys
    ct10 = np.array(ct1[0].F, dtype=np.int64)
    ct11 = np.array(ct1[1].F, dtype=np.int64)

    ct20 = np.array(ct2[0].F, dtype=np.int64)
    ct21 = np.array(ct2[1].F, dtype=np.int64)

    # print("Fresh Shapes", ct10.shape, ct11.shape, ct20.shape, ct21.shape)

    #Saving Encrypted Data
    print("Saving Encrypted Data and Relin Keys")
    ...
    #ct1 save
    outfile = open("bins_demo/ct10_fresh.bin", mode='wb')
    ct10.tofile(outfile)
    outfile.close

    outfile = open("bins_demo/ct11_fresh.bin", mode='wb')
    ct11.tofile(outfile)
    outfile.close

    #ct1 save
    outfile = open("bins_demo/ct20_fresh.bin", mode='wb')
    ct20.tofile(outfile)
    outfile.close

    outfile = open("bins_demo/ct21_fresh.bin", mode='wb')
    ct21.tofile(outfile)
    outfile.close
    
    #rlk1 save
    relinKey = np.array(Evaluator.rlk_store, dtype=np.int64)
    # print("relinKey", relinKey.shape)
    # print(relinKey)
    outfile = open("bins_demo/relinKey.bin", mode='wb')
    relinKey.tofile(outfile)
    outfile.close
...
def decrypt(Evaluator, n, q, qnp, n1, n2):
    ...
    # print result    
    if nr == n1 * n2:
        print("Correct decryption of cypertext of value ", n1, "*",  n2, " = ", nr)
        print()
        return True
    else:
...
def cleanup(ask = False):
    while True and ask:
        choice = input("Do you want to run the cleanup script? (yes/no): ").lower()
...
# For the demo this should be done using our HDL implementation
def compileHEmulSimulation_C():
    # Compile the C program
    print("Compiling HEmul.c")
    compile_command = ["gcc", "-o", "demoHEmul", "demoHEmul.c", "-lm", "-lgmp"]
    subprocess.run(compile_command, check=True)
    
def runHEmulSimulation_C():
    # Compile the C program
    print("Compiling HEmul.c")
    compile_command = ["gcc", "-o", "demoHEmul", "demoHEmul.c", "-lm", "-lgmp"]
    subprocess.run(compile_command, check=True)
...
def main():
    ...
    print("Welcome to the HE Demo: just the encryption and decryption can be done by passing the aurgument 'e' or 'd' to the script")    
        
    # en/dycryption relies on the seed
    random.seed(42)
    
    t, n, logq, T = 64, 64, 32, 16
    q, psi, psiv, w, wv = 4294966657, 766107853, 3158179387, 4208251219, 1307994966
    
    # hardcoded numbers for the debug
    n1, n2 = 6, 7

    Evaluator, qnp = initialize(t, n, logq, q, psi, psiv, w, wv, T)
    
    if mode == "e" or len(sys.argv) == 1:
        print("Running encryption")
        
        #only for the demo
        if len(sys.argv) == 1:
            n1, n2 = select_numbers(notSafe = False)
 
        encrypt(Evaluator, n1, n2)
        
        if mode == "e":
            # compile the C program for the debugging
            compileHEmulSimulation_C()
            sys.exit(1)
    
    if len(sys.argv) == 1:
        print("Press Enter to multiply your numbers in the cyphertext domain")
        input()
    
        # For the demo this should be done using our HDL implementation
        compileHEmulSimulation_C()
        runHEmulSimulation_C()

    if mode == "d" or len(sys.argv) == 1:
        print("Running decryption")
        vaild = decrypt(Evaluator, n, q, qnp, n1, n2)
        
        if mode == "d":
            sys.exit(1)
    
    if mode == "c" or len(sys.argv) == 1:
        cleanup()                
